    ...
    for numBloc = 1:numBlocks
        ...
        if dummymode == 0 && godMode == 2
            ...
            while calibrationNotOk
                ...
                trialFunction(Answer, emotionalCategories, emotionalCategoriesFr, 'GazeVerif', fakeCountSide, TotalListTraining, imageFolderTraining, globalTask, 'Training', timeBetweenTrials, 1, 1, 'None', gazeInf);
                proposeCalibration()
                %disp(RedoGazeTest);
        ...
        for trialNum = 1:nTrialsPerBlock
            ...
            if ~trialDone
                [trialCounter, ListBloc, countSide] = trialFunction(Answer, emotionalCategories, emotionalCategoriesFr, trialCounter, countSide, ListBloc, imageFolder, globalTask, task, timeBetweenTrials, 1, 0, false(1));
            end
            %}
            if strcmp(optiSeq{trialNum}, 'Dummy')
                dummyFunction(timeBetweenTrials, task, globalTask);
            else
                [trialCounter, ListBloc, countSide] = trialFunction(Answer, emotionalCategories, emotionalCategoriesFr, trialCounter, countSide, ListBloc, imageFolder, globalTask, task, timeBetweenTrials, 1, 0, optiSeq{trialNum}, false(1));
            end
        end
